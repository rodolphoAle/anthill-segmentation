# Metodologia  Técnicas Aplicadas no Projeto
...
>
> **Padrão de cada técnica:** nome técnico + explicação acessível em pt-BR + motivação no contexto deste projeto + run em que entrou.
...
9. [Resumo Cronológico de Decisões](#9-resumo-cronológico-de-decisões)
...
| **Poucos exemplos positivos**  apenas ~600 tiles em 9.862 contêm formigueiros (6%) | Modelo decora os exemplos disponíveis se não houver augmentação |
...
**O que é (Run 11+):** descartar do conjunto de treino tiles cujas máscaras tenham mais de 70% de pixels brancos (área não rotulada).

**Como funciona (simples):** ao construir o dataset, o código abre cada máscara e calcula a fração de pixels brancos. Se mais de 70% da imagem é "ignorar", a tile é descartada. **O conjunto de validação não é filtrado**  assim métricas continuam comparáveis com runs anteriores.
...
**Resultado:** das 9.862 tiles iniciais, 1.713 (17.4%) foram descartadas, mantendo 8.149 tiles "úteis". Cada epoch ficou ~17% mais rápida.
...
**Quando entrou:** Run 02 em diante.
...
**Por que aplicado:** simula pequenas variações de captura do drone/avião  ortofotos nem sempre são geometricamente perfeitas. Pequenos ângulos preservam o conteúdo central.
...
### 3.3 Rotação 90° (testada e descartada)

**O que é (Run 08):** rotação aleatória entre 90°, 180°, 270° (preservação geométrica perfeita, sem pixels novos).

**Por que descartada (Run 09+):** apesar de matematicamente segura, gerou orientações que **não existem** no conjunto de validação. As ortofotos são geralmente alinhadas com pontos cardeais (norte=cima), e a meta é reconhecer formigueiros em fotos novas, idealmente nessa mesma convenção. Rotacionar 90° ensina o modelo a reconhecer formigueiros "de lado", capacidade que não é testada e potencialmente prejudica a especialização.

---

### 3.4 Color Jitter (variação fotométrica)
...
### 3.5 Elastic Transform (deformação elástica)
...
**Histórico:**
- Run 08: alpha=50, sigma=5 (agressivo demais, criava texturas irrealistas)
- Run 09+: alpha=25, sigma=4 (deformação mais sutil, realista)

---

### 3.6 Copy-Paste Augmentation
...
#### 3.6.1 Implementação original (Run 08)
...
4. Recorta a região do formigueiro (com 5px de padding) e cola na imagem negativa em posição aleatória.
5. Atualiza a máscara para marcar a região colada como classe 1 (formigueiro).

**Por que aplicado:** apenas multiplicar/repetir os 600 doadores via oversampling (Runs 06-07) ensinou o modelo a decorar os mesmos exemplos. Copy-paste cria **combinações genuinamente novas**  formigueiros do dataset 1 colados em fundos do dataset 2.

**Quando entrou:** Run 08 (probabilidade p=0.5).

**Histórico de probabilidade:**
- Run 08: p=0.5 (agressivo demais  modelo aprendeu artefatos de colagem)
- Run 09: p=0.1 (muito conservador  só ~60 sintéticos/epoch)
- Run 11: p=0.4 (balanço ajustado)

#### 3.6.2 Melhorias do Run 11

Diagnóstico do Run 10 mostrou três falhas qualitativas no copy-paste original. Cada uma foi corrigida:

##### **P1  Bordas suaves (Gaussian alpha blending)**

**Problema:** o paste usava `np.where(máscara_binária, doador, fundo)`  transição brusca de 1 pixel entre doador e fundo. Visualmente aparecia como uma "silhueta recortada". O modelo aprendia esse padrão como atalho ("se há borda nítida, é formigueiro") em vez de aprender o padrão visual real.
...
**Por que isso funciona:** o modelo é forçado a aprender a fronteira **semântica** do formigueiro (texturas, cores, formas reais) em vez da fronteira **artificial** de colagem.
...
**Problema:** os ~600 doadores apareciam sempre na mesma orientação. Com p=0.4 e 50 epochs, cada doador apareceria ~33 vezes idêntico → memorização.
...
### 4.2 Batch Normalization (Run 05+)

**O que é:** camada de normalização que padroniza as ativações dentro de cada batch durante o treinamento.

**Como funciona (simples):** após cada convolução, a camada calcula a média e o desvio das ativações **dentro do batch** e normaliza. Equivalente a "centralizar e escalar" os números antes de passar adiante.

**Por que aplicado:**
- **Estabiliza o treinamento:** sem BatchNorm, gradientes podem explodir ou sumir em redes profundas (Run 04 mostrou instabilidade clara  val_loss travada em platô)
- **Permite learning rate maior:** com normalização, o modelo aceita LR=1e-3 sem explodir
- **Atua como regularizador leve:** o ruído natural da estatística por batch substitui parcialmente outras formas de regularização

**Quando entrou:** Run 05.

---

### 4.3 ConvTranspose2d vs Bilinear Upsampling (Run 05+)

**O que é:** método de "aumentar a resolução" no decoder da U-Net.

**Como funciona (simples):**
- **Bilinear (Run 01-04):** interpolação fixa, similar ao zoom de uma imagem em editor  calcula pixels intermediários por média ponderada dos vizinhos. **Não tem parâmetros aprendíveis.**
- **ConvTranspose2d (Run 05+):** "convolução transposta"  operação aprendível que aumenta a resolução. **Tem parâmetros que o gradiente ajusta.**

**Por que aplicado:** com ConvTranspose2d, o modelo aprende **como** reconstruir a resolução de modo otimizado para a tarefa, não apenas "esticar" a imagem. Em conjunto com BatchNorm, este foi um dos saltos qualitativos mais significativos do projeto (Run 04 → Run 05).
...
### 5.1 Cross-Entropy Loss + class weights (Run 01-02)
...
**Class weights:** multiplica a penalidade da classe formigueiro por um fator (10.0 no Run 01, 6.0 no Run 02) para compensar o desbalanceamento.

**Por que abandonada após Run 02:** mesmo com class weights, a Cross-Entropy é "preguiçosa"  convergir para "tudo fundo" minimiza a loss média rapidamente sem aprender a detectar formigueiros de fato.

---

### 5.2 Focal Loss (Run 03+)
...
- α=0.1, β=0.9 → ainda mais agressivo no Recall (Run 09-10, abandonado)
...
**Histórico de β:**
- Run 04: 0.7 (moderado)
- Run 05: 0.6 (mais conservador  melhor IoU)
- Run 06-08: 0.8 (aumentar Recall)
- Run 09-10: 0.9 (Recall máximo, mas degradou IoU)
- Run 11: 0.7 (volta ao balanço)

---

### 5.4 Combined Tversky+Focal Loss (Run 04+)
...
**Por que combinada:** Tversky pura **colapsa** no início do treino quando o desbalanceamento é extremo  o termo `α·FP` domina o denominador, o gradiente empurra `prob(formigueiro) → 0` em todos os pixels, e a loss trava num platô. Isso aconteceu na primeira tentativa do Run 04. Solução: usar Focal como **âncora** (estabiliza o início) e Tversky como **direcionador** (otimiza Recall).

**Onde:** [app/service/training_service.py:121-176](app/service/training_service.py).

---

### 5.5 Lovász Hinge Loss (Run 11+)
...
**Por que aplicado:** no Run 10, β=0.9 do Tversky empurrou Recall para 85.2% mas IoU regrediu de 35.2% (Run 05) para 30.3%. A Lovász foi adicionada para "puxar de volta" a precisão da máscara  IoU é o que a métrica final mede, então otimizar diretamente para isso faz sentido.

**Loss combinada do Run 11:**
```
...
#### 6.2.1 ReduceLROnPlateau (Run 01-06)
...
**Por que abandonado (Run 07+):** ReduceLR depende de uma val_loss "limpa" para detectar plateaus. Quando o WeightedRandomSampler (Run 06) foi adicionado, a val_loss ficou ruidosa (cada epoch via uma composição diferente de exemplos upsampled), e o scheduler reduzia o LR cedo demais, congelando o aprendizado.

#### 6.2.2 CosineAnnealingLR (Run 07+)
...
**Configuração atual:** `T_max=50` epochs, `eta_min=1e-6`.
...
**Por que aplicado:** sem BatchNorm (Runs 01-04), gradientes podem explodir e produzir Inf/NaN, corrompendo os pesos. Com BatchNorm (Run 05+), o problema é menor mas o clipping continua como rede de segurança.
...
- **NaN/Inf de outputs:** podem aparecer durante instabilidade numérica (resolvido por BatchNorm + clipping, mas o skip continua como segurança)
...
### 6.5 WeightedRandomSampler 3:1 (testado e descartado)

**O que era (Run 06-07):** sampler que pesava as tiles positivas 3× mais nas amostragens, fazendo cada batch ter ~3× mais exemplos positivos do que a distribuição natural.

**Por que descartado:** repetia os mesmos 600 exemplos positivos múltiplas vezes por epoch, ensinando o modelo a decorar em vez de generalizar. Recall não melhorou; pelo contrário, a val_loss ficou volátil (efeito do oversampling) e o ReduceLROnPlateau congelou o aprendizado cedo.

**Substituído por:** Copy-Paste Augmentation (Run 08+), que cria variabilidade real em vez de repetir.
...
**Configuração atual:** 0.5 (Run 06+).

**Histórico:** Run 03 começou em 0.7 (muito conservador, perdia formigueiros). Run 05 reduziu para 0.6 e ganhou Recall. Runs 06+ usam 0.5.
...
## 9. Resumo Cronológico de Decisões

> Síntese: quando cada técnica entrou no projeto e por quê.

| Run    | Técnica adicionada                                  | Motivação                                                    |
| ------ | --------------------------------------------------- | ------------------------------------------------------------ |
| 01     | Baseline U-Net + Cross-Entropy + class_weight=10    | Estabelecer linha de base                                    |
| 02     | Augmentações geométricas + fotométricas             | Reduzir overfitting (Run 01 decorou o dataset)              |
| 02+    | Threshold de confiança + filtro de região mínima    | Limpar predições ruidosas pós-treino                         |
| 03     | Focal Loss (γ=2.0)                                  | Forçar foco em casos difíceis (solo ambíguo)                |
| 04     | Tversky Loss + Combined Tversky+Focal               | Penalizar FN mais que FP; evitar colapso de Tversky pura     |
| 05     | BatchNorm + ConvTranspose2d                         | Estabilizar gradiente; aprender upsampling                   |
| 06     | WeightedRandomSampler 3:1                           | Aumentar exposição a positivos (FRACASSO  abandonado)      |
| 07     | CosineAnnealingLR                                   | Substituir ReduceLROnPlateau (volátil com sampler)           |
| 08     | Copy-Paste Augmentation + RandomRotate90 + Elastic  | Criar variabilidade real (não repetição)                     |
| 09     | Tversky β=0.9, Elastic alpha=25, p_copy_paste=0.1   | Suavizar augmentações agressivas do Run 08                   |
| 10     | (mesmos parâmetros do Run 09, completando 100 ep.)  | Verificar se Run 09 (interrompido) melhoraria com mais epochs|
| **11** | **Lovász Hinge Loss + filtro de tile + copy-paste melhorado (P1+P2+P3)** | **Recuperar IoU perdido no Run 10**       |

### Princípios extraídos da trajetória

1. **Loss e arquitetura são interdependentes**  Tversky não funcionou bem sem BatchNorm (Run 04 → Run 05).
2. **Augmentação realista > augmentação agressiva**  Run 08 mostrou que augmentações artificiais (alpha=50, rotate90 da imagem) prejudicam generalização.
3. **Repetir dados não substitui criar dados**  WeightedRandomSampler (Run 06) falhou; Copy-Paste (Run 08) começou a funcionar.
4. **Métrica errada otimizada gera regressão**  Run 10 maximizou Recall via Tversky β=0.9 mas degradou IoU. Solução: otimizar IoU diretamente via Lovász (Run 11).
5. **Limpeza de dados é tão importante quanto modelagem**  filtrar tiles dominadas por borda branca (Run 11) deu 17% de speedup gratuito + melhor sinal de gradiente.
...
## Referências Bibliográficas Implícitas

> As técnicas listadas neste documento são fundamentadas nos seguintes trabalhos. Esta é uma lista de referência  a tese principal terá citações formatadas.
