...
| **Normalização**             | GroupNorm (8 grupos) após cada convolução  |
...
| **14** | **U-Net + GN + ConvT2d** | **Tversky+Focal+Lovász (50/20/30)** | **Geom + Foto + Anthill Duplicate (p=0.7)** | **75.9%** | **91.7%** | **83.1%** | **33.8%** | **Maior Recall do projeto; melhor threshold calibrado; normalização migrada para GroupNorm** |
...
1. **Normalização + ConvTranspose2d foram determinantes** — a Run 05 (primeira com normalização nas convoluções, na época BatchNorm, mais ConvTranspose2d) estabeleceu o melhor IoU histórico (35.2%). Posteriormente, devido ao batch pequeno (=2) imposto pela VRAM, o BatchNorm foi migrado para **GroupNorm (8 grupos)**, usado no modelo final (Run 14), que recuperou 33.8% — o mais próximo desse valor em runs com augmentações avançadas.
